...
class AttendanceProcessor(IAttendanceProcessor):
    ...
    def process_event(self, event: AttendanceEventEntity) -> ProcessEventResult:
        ...
        recent = self._events.get_recent_events(employee_id=employee_id, since=since)
        for r in recent:
            if r.id != event.id and r.event_type == event.event_type:
                # Build a minimal placeholder record entity for the response
                existing_record = self._records.get_by_employee_and_date(employee_id, event_date)
        ...
        record_model, is_new = self._records.get_or_create_for_date(
        ...
        # 10. Save
        entity = self._records.save_model(record_model)
