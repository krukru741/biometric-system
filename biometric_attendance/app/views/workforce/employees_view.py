...
class EmployeesView(QWidget):
    ...
    def __init__(self, view_model, parent=None):
        ...
        self._setup_ui()
        self._connect_signals()
        QTimer.singleShot(0, self.vm.load_data)
    ...
    def _setup_ui(self):
        ...
        layout.addWidget(PageHeader("Employees", "Find people, review their profiles, and manage employment details.", action=self.add_btn))
        ...
        filter_layout.add_field("&Search employees", self.search_input)
        filter_layout.add_field("&Status", self.status_filter)
        filter_layout.add_field("&Department", self.dept_filter)
        filter_layout.add_field("&Position", self.pos_filter)
        layout.addWidget(filter_layout)

        columns = [
            Column("Employee ID", lambda e: e.employee_id),
            Column("Name", lambda e: e.full_name, width=220),
            Column("Department", lambda e: e.department_name, width=180),
            Column("Position", lambda e: e.position_name, width=180),
            Column("Type", lambda e: e.employment_type.value if e.employment_type else None),
            Column("Status", lambda e: e.status.value if e.status else None),
            Column("Email", lambda e: e.email, width=240),
        ]
        self.table = RecordTable(columns, name="Employees", row_key=lambda e: e.id)
        self.content_state = ContentState(self.table)
        self.content_state.show_state("loading", "Loading employees…")
        layout.addWidget(self.content_state, 1)
        actions = QHBoxLayout()
        self.result_label = plain_label("Select an employee to view or edit their profile.")
        actions.addWidget(self.result_label, 1)
        self.view_btn = QPushButton("View profile")
        self.edit_btn = QPushButton("Edit")
        self.archive_btn = QPushButton("Archive")
        for button in (self.view_btn, self.edit_btn, self.archive_btn):
            button.setEnabled(False)
            button.setMinimumHeight(36)
            actions.addWidget(button)
        layout.addLayout(actions)
        self.table.selectionModel().currentChanged.connect(self._on_selection_changed)
        self.table.activated.connect(lambda _: self._selected_action(self._on_view_clicked))
        self.view_btn.clicked.connect(lambda: self._selected_action(self._on_view_clicked))
        self.edit_btn.clicked.connect(lambda: self._selected_action(self._on_edit_clicked))
        self.archive_btn.clicked.connect(lambda: self._selected_action(self._on_archive_clicked))
        self.content_state.action_requested.connect(self.vm.load_data)
    ...
    def _render_table(self, employees):
        self.table.set_rows(employees)
        self.result_label.setText(f"{len(employees)} employees · Select a row for actions")
        if employees:
            self.content_state.show_content()
        else:
            self.content_state.show_state("empty", "No employees found", "Try changing your search or filters, or add an employee.")
        self._on_selection_changed()

    def _on_selection_changed(self, *args):
        employee = self.table.selected_record()
        available = employee is not None and not self._loading and self.content_state.state == "content"
        self.view_btn.setEnabled(available)
        self.edit_btn.setEnabled(available)
        self.archive_btn.setEnabled(available and employee.status != EmploymentStatus.ARCHIVED)
        for button in (self.view_btn, self.edit_btn, self.archive_btn):
            button.setAccessibleName(f"{button.text()} {employee.full_name}" if employee else button.text())

    def _selected_action(self, action):
        employee = self.table.selected_record()
        if employee is not None and not self._loading:
            action(employee)
    ...
    def _on_error(self, message):
        self.content_state.show_state("error", "Employees unavailable", message, action="Try again")
        self._on_selection_changed()
